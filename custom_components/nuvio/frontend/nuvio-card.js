  ...
  sourcesView(){
    ...
    var groupHtml=[...groups.entries()].map(([addon,items])=>{
      ...
      var rows=items.map(({stream:s,index:n})=>{
        ...
        var linkHtml=directUrl
          ? '<div class="source-url-wrap"><a class="source-url openlink" data-source-index="'+n+'" href="'+this.esc(directUrl)+'" target="_blank" rel="noreferrer">'+this.esc(directUrl)+'</a></div>'
          : torrentLink
            ? '<div class="source-url-wrap"><span class="source-url torrent-url">'+this.esc(torrentLink)+'</span></div>'
            : s.info_hash
        ...
        var actionHtml=s.direct
          ...
          : '<span class="resolver">'+this.esc(unavailable)+'</span>'+(torrentLink?'<button class="action copytorrent" data-source-index="'+n+'">Copy magnet</button>':"");
    ...
    return '<div class="top source-top"><button class="back" id="backDetails">← '+this.esc(title)+'</button><h3>Sources · '+this.esc(sub)+'</h3><div class="controls">'+this.playerSelect()+'</div><div class="source-summary">'+this._streams.length+' source'+(this._streams.length===1?"":"s")+' from '+groups.size+' addon'+(groups.size===1?"":"s")+'</div></div>'+
...
console.info("NUVIO-CARD v0.3.9");
