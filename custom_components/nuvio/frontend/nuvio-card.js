class NuvioCard extends HTMLElement {
  ...
  async openCollection(folder,section){
    ...
    this._catalogPaging=null;this._catalogVisibleCount=0;this._catalogLoadingMore=false;this._resetCatalogScroll=true;
  ...
  async openCatalog(section){
    ...
    this._catalogVisibleCount=Math.min(this.catalogChunkSize(),this._catalogItems.length);
    this._catalogLoadingMore=false;this._resetCatalogScroll=true;
    this._catalogLoading=true;this._view="catalog";this._error="";this.render();
    try{
  ...
  async loadMoreCatalog(){
    ...
    if(this._catalogItems.length>=300)return;
    ...
    try{
      if(this._collectionTabs){
        ...
        if(!tabs.length){this._catalogLoadingMore=false;this.render();return;}
        ...
        this._error=active.filter(t=>t.error).map(t=>t.name+": "+t.error).join(" · ");
      ...
      if(requestId===this._catalogRequest)this._error=e.message||"Could not load more titles.";
    }finally{
      if(requestId===this._catalogRequest){this._catalogLoadingMore=false;this.render();}
    }
  }
  ...
  header(){
    ...
    return '<div class="header"><div class="header-title"><h2>'+this.esc(this._config.title||"Nuvio")+'</h2><span class="card-version">v0.4.55</span></div><div class="tools">'+search+
      home+
...
console.info("NUVIO-CARD v0.4.55");
