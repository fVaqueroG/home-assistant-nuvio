class NuvioCard extends HTMLElement {
  ...
  watchProviderRow(){
    ...
    var scope=meta.scope==="season"
      ? "Season availability"
      : "Movie availability";
    var cards=providers.map(provider=>{
      var index=this.watchHubSourceIndex(provider);
      var playable=index>=0;
      var access=(provider.access||[]).join(" · ");
      var hint=playable
        ? "Play with "+provider.name
        : provider.name+" is available here, but WatchHub did not provide a direct provider link.";
      ...
      return '<button class="watch-provider '+(playable?"linked":"availability-only")+'"'+
        (playable?' data-provider-source-index="'+index+'"':" disabled")+
        ' title="'+this.esc(hint)+'">'+
          logo+
  ...
  wire(){
    ...
    r.querySelectorAll("[data-provider-source-index]").forEach(b=>b.addEventListener("click",()=>this.playInNuvioIndex(Number(b.dataset.providerSourceIndex))));
...
console.info("NUVIO-CARD v0.4.35");
