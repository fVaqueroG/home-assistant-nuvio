...
function nuvioUseMobileLayout(element) {
  const width=Number(element?.getBoundingClientRect?.().width)||0;
  const media=typeof window!=='undefined'&&typeof window.matchMedia==='function'
    ? window.matchMedia('(max-width:700px), (pointer:coarse), (orientation:portrait) and (max-width:900px)').matches
    : false;
  return (width>0&&width<=700)||Boolean(media);
}
...
class NuvioCard extends HTMLElement {
  ...
  static getStubConfig(){ return {title:"Nuvio",columns:6,show_remote:true,remote_side:"left",theme:"system",accent_preset:"indigo",accent_color:"#6157FB"}; }
  _refreshCardTheme(){
  ...
  if (this._remoteExpanded && portal) {
    ...
    nuvioThemeStyle(portal,mode,darkMode,this._config.accent_preset,this._config.accent_color);
    portal.dataset.nuvioTheme=mode==='light'||mode==='dark' ? mode : (darkMode ? 'dark' : 'light');
  }
}
setConfig(c){
  this._config=Object.assign({title:"Nuvio",columns:6,show_search:true,show_remote:true,remote_side:"left",theme:"system",accent_preset:"indigo",accent_color:"#6157FB"},c||{});
...
async choosePlayer(player){
  ...
  catalogChunkSize(){
    var mobile=nuvioUseMobileLayout(this);
  ...
  connectedCallback(){
    this.syncHeroTimer();
    if(!this._nuvioResponsiveHandler&&typeof window!=='undefined'){
      this._nuvioMobileLayout=nuvioUseMobileLayout(this);
      this._nuvioResponsiveHandler=()=>{
        const next=nuvioUseMobileLayout(this);
        if(next===this._nuvioMobileLayout)return;
        this._nuvioMobileLayout=next;
        if(this.isConnected)this.render();
      };
      window.addEventListener('resize',this._nuvioResponsiveHandler,{passive:true});
      window.addEventListener('orientationchange',this._nuvioResponsiveHandler,{passive:true});
      if(typeof ResizeObserver==='function'){
        this._nuvioResizeObserver=new ResizeObserver(this._nuvioResponsiveHandler);
        this._nuvioResizeObserver.observe(this);
      }
    }
  }
  header(){
    ...
    var beginning=["home","catalog","sources"].includes(this._view)
  ...
  var home='<button class="ib toolbar-btn '+(onHome?"toolbar-active":"")+'" id="homeTop" title="Home" aria-label="Home"><ha-icon icon="mdi:home"></ha-icon></button>';
    return '<div class="header"><div class="header-title"><img class="nuvio-wordmark" src="/nuvio/assets/wordmark.png?v=0.4.99" alt="Nuvio" decoding="async" style="display:none;height:38px;max-width:150px;width:auto;object-fit:contain"><h2 class="nuvio-wordmark-fallback">'+this.esc(this._config.title||"Nuvio")+'</h2><span class="card-version">v0.4.99</span></div><div class="tools">'+search+this.roomSelect()+'<label class="toolbar-player" title="Select media player"><ha-icon icon="mdi:television" aria-hidden="true"></ha-icon>'+this.playerSelect()+'</label>'+
      home+
      '<button class="ib toolbar-btn" id="refresh" title="Refresh" aria-label="Refresh"><ha-icon icon="mdi:refresh"></ha-icon></button>'+
      addons+'<div class="nuvio-header-nav">'+beginning+remote+'</div></div>';
  }
  async goHome(){
    this._view="home";
    this._error="";
    ...
    portal.innerHTML=`
      <style>
        ...
        #nuvio-remote-portal .nuvio-ring-ok{
          ...
          background:var(--nuvio-primary,#6157fb);border:0;color:var(--nuvio-primary-ink,#fff);font-size:11px;font-weight:800;
        ...
        #nuvio-remote-portal button:focus-visible{outline:2px solid var(--nuvio-focus-color,#23dcfd);outline-offset:2px}\n        #nuvio-remote-portal button:active{transform:scale(.96)}
  ...
  disconnectedCallback(){
    ...
    if(this._lazyObserver){this._lazyObserver.disconnect();this._lazyObserver=null;}
    if(this._catalogObserver){this._catalogObserver.disconnect();this._catalogObserver=null;}
    if(this._nuvioResizeObserver){this._nuvioResizeObserver.disconnect();this._nuvioResizeObserver=null;}
    if(this._nuvioResponsiveHandler&&typeof window!=='undefined'){
      window.removeEventListener('resize',this._nuvioResponsiveHandler);
      window.removeEventListener('orientationchange',this._nuvioResponsiveHandler);
      this._nuvioResponsiveHandler=null;
    }
    this.removeRemotePortal();
  }
  detailsView(){
  ...
  wire(){
    ...
    r.querySelectorAll(".nuvio-scroll-start").forEach(button=>button.addEventListener("click",()=>{
      this.shadowRoot.querySelector(".nuvio-home-scroll, .catalog-scroll, .nuvio-sources-scroll")?.scrollTo({top:0,behavior:"smooth"});
    }));
  ...
  render(){
    if(!this.shadowRoot)return;
    var oldHomeScroller=this.shadowRoot.querySelector(".nuvio-home-scroll");
    if(oldHomeScroller)this._nuvioHomeScrollTop=oldHomeScroller.scrollTop;
    var oldSourcesScroller=this.shadowRoot.querySelector(".nuvio-sources-scroll");
    ...
    if(this._view==="home")body='<div class="nuvio-home-scroll">'+body+'</div>';
    if(this._view==="sources")body='<div class="nuvio-sources-scroll">'+body+'</div>';
    ...
    var homeScroller=this.shadowRoot.querySelector(".nuvio-home-scroll");
    if(homeScroller)homeScroller.scrollTop=this._nuvioHomeScrollTop||0;
    var sourcesScroller=this.shadowRoot.querySelector(".nuvio-sources-scroll");
...
class NuvioCardEditor extends HTMLElement {
  ...
  render(){
    ...
    const accentPreset=Object.hasOwn(NUVIO_ACCENTS,String(cfg.accent_preset||''))||cfg.accent_preset==='custom'?cfg.accent_preset:'indigo';
    const accentColor=nuvioNormalizeHex(cfg.accent_color||NUVIO_ACCENTS.indigo.hex);
    const accentOptions=Object.entries(NUVIO_ACCENTS).map(([key,item])=>'<option value="'+key+'" '+(accentPreset===key?'selected':'')+'>'+this.esc(item.label)+' · '+item.hex+'</option>').join('')+
      '<option value="custom" '+(accentPreset==='custom'?'selected':'')+'>Custom</option>';
    this.shadowRoot.innerHTML=`<style>
      ...
      select,input[type=text],input[type=number],input[type=color]{box-sizing:border-box;width:100%;min-width:0;padding:10px;border:1px solid var(--divider-color);border-radius:9px;background:var(--secondary-background-color);color:var(--primary-text-color);font:inherit}
      ...
      button.add{background:var(--primary-color);color:var(--nuvio-primary-ink,#fff);border-color:transparent}
      ...
      <section class="group"><h3>General</h3><div class="fields">
        ...
        <label>Accent color<select data-field="accent_preset">${accentOptions}</select><small>Changes Nuvio controls and selected states without altering the logo.</small></label>
        ${accentPreset==='custom'?'<label>Custom accent<input type="color" data-field="accent_color" value="'+accentColor+'"><small>Used only when Accent color is Custom.</small></label>':''}
...
console.info("NUVIO-CARD v0.4.99");
...
launcherStyle() {
  ...
  if (style === "icon_text") {
    ...
    if (style === "vertical") {
      ...
      logo.src = "/nuvio/assets/vertical.png?v=0.4.99";
    } else if (style === "logo_only") {
      logo.className = "launcher-mark";
      logo.src = "/nuvio/assets/icon-only.png?v=0.4.99";
    } else {
      logo.className = "launcher-logo";
      logo.src = "/nuvio/assets/wordmark.png?v=0.4.99";
    }
    logo.alt = "";
...
(() => {
  ...
  function step(card) {
    ...
    if (card._view === 'search' || card._view === 'catalog') { if (card._view === 'catalog') card._catalogRequest = (card._catalogRequest || 0) + 1; void card.goHome(); return true; }
    return false;
  }
