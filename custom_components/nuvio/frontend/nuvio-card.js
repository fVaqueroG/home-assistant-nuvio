class NuvioCard extends HTMLElement {
  constructor(){
    ...
    this._sections=[]; this._results=[]; this._view="home"; this._item=null; this._details=null; this._season=null; this._query=""; this._error="";
  }
  ...
  players(){ return this._hass?Object.keys(this._hass.states).filter(x=>x.startsWith("media_player.")).sort():[]; }
  player(){ var s=this.shadowRoot&&this.shadowRoot.querySelector("#player"); return (s&&s.value)||this._config.default_player||this._config.entity||this.players()[0]||""; }
  async loadHome(){
    if(!this._hass)return; this._loading=true; this._error=""; this.render();
    try{ var r=await this.ws({type:"nuvio/home"}); this._sections=r.sections||[]; this._loaded=true; }
  ...
  playerSelect(){
    var configured=this._config.default_player||this._config.entity||"";
    return '<select id="player">'+this.players().map(id=>'<option value="'+this.esc(id)+'" '+(id===configured?"selected":"")+'>'+this.esc((this._hass.states[id].attributes||{}).friendly_name||id)+'</option>').join("")+'</select>';
  }
  detailsView(){
...
console.info("NUVIO-CARD v0.3.0");
