...
(async()=>{
 ...
 const movieCalls=[];movieAuto.ws=async m=>{movieCalls.push(m);if(m.type==='nuvio/details')return {id:m.content_id,type:'movie',name:'Movie details',releaseInfo:'2026',genres:['Drama','Sci-Fi'],description:'Movie overview text',poster:'https://img.example/movie.jpg',videos:[]};if(m.type==='nuvio/streams')return {streams:m.addon_scope==='watchhub'?[{addon:'WatchHub',name:'Netflix',provider_key:'netflix',external_url:'https://www.netflix.com/title/test',external:true,direct:false,badges:[]}]:[{addon:'Direct',name:'Movie stream',url:'https://cdn.example/movie.mp4',direct:true,badges:[]}],debrid:{configured:false}};if(m.type==='nuvio/watch_providers')return {configured:true,tmdb_configured:true,tvdb_configured:true,region:'US',scope:'movie',providers:[{provider_id:8,name:'Netflix',provider_key:'netflix',logo_url:'https://img.example/netflix.jpg',access:['Subscription'],deep_link:'https://www.netflix.com/title/exact',deep_link_source:'thetvdb'},{provider_id:337,name:'Disney Plus',provider_key:'disney',logo_url:'https://img.example/disney.jpg',access:['Subscription'],deep_link:'https://www.disneyplus.com/browse/entity-exact',deep_link_source:'thetvdb'}],attribution:'Availability by JustWatch via TMDB · Exact provider links via TheTVDB'};throw new Error('Unexpected '+m.type);};
