...
(async()=>{
 ...
 const movieCalls=[];movieAuto.ws=async m=>{movieCalls.push(m);if(m.type==='nuvio/details')return {id:m.content_id,type:'movie',name:'Movie details',releaseInfo:'2026',genres:['Drama','Sci-Fi'],description:'Movie overview text',poster:'https://img.example/movie.jpg',videos:[]};if(m.type==='nuvio/streams')return {streams:m.addon_scope==='watchhub'?[{addon:'WatchHub',name:'Netflix',external_url:'https://www.netflix.com/title/test',external:true,direct:false,badges:[]}]:[{addon:'Direct',name:'Movie stream',url:'https://cdn.example/movie.mp4',direct:true,badges:[]}],debrid:{configured:false}};throw new Error('Unexpected '+m.type);};
 ...
 assert.equal(movieAuto.shadowRoot.querySelector('.header h2').textContent.trim(),'Nuvio');assert.ok(movieAuto.shadowRoot.querySelector('#refresh'));assert.ok(movieAuto.shadowRoot.querySelector('#addonsToggle'));
 ...
 const episodeProviderCalls=[];seriesAuto._hass={states:{'media_player.tv':{}},callService:async(...args)=>episodeProviderCalls.push(args)};seriesAuto._playerId='media_player.tv';seriesAuto.render();seriesAuto.shadowRoot.querySelector('.nuvioplay').click();await flush();assert.equal(episodeProviderCalls.length,1);const episodeProviderData=episodeProviderCalls[0][2];assert.equal(episodeProviderData.media_type,'series');assert.equal(episodeProviderData.content_id,'series1');assert.equal(episodeProviderData.video_id,'series1:1:1');assert.equal(episodeProviderData.season,1);assert.equal(episodeProviderData.episode,1);assert.equal(episodeProviderData.episode_title,'Pilot');assert.match(seriesAuto.shadowRoot.textContent,/Series-level/);
